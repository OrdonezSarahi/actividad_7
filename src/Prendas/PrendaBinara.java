...
public class PrendaBinara {
    ...
    public PrendaBinara(String nombreArchivo){
    ...
    public void guardarDatos(ArrayList<Prenda> prendas) throws FileNotFoundException {
        ...
        try{
            for(Prenda p : prendas){
                output.writeUTF(p.getModelo());
                output.writeUTF(p.getTela());
                output.writeDouble(p.getCostoProduccion());
                output.writeUTF(p.getGenero());
                output.writeUTF(p.getTemporada());
            }
        }catch(IOException e){}
        try{
            output.close();
    ...
    public ArrayList<Prenda> leerDatos() throws FileNotFoundException {
        ...
        try{
            while(true){
                ...
                String tela = input.readUTF();
                double costo = input.readDouble();
                String genero = input.readUTF();
                String temporada = input.readUTF();
                prendas.add(new Prenda(modelo, tela, costo, genero, temporada));
            }
        }catch(Exception e){}
        try{
            input.close();
